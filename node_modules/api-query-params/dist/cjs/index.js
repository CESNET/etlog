...
const parseValue = (value, key, options) => {
  ...
  const fullDate = value.match(/([12]\d{3})-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])(T| )?([01][0-9]|2[0-3]):([0-5]\d):([0-5]\d)(\.(\d{3})Z)?/);
  ...
  const shortDate = value.match(/([12]\d{3})-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])/);
  /* eslint-enable max-len */

  if (shortDate) {
    return new Date(shortDate[1], Number(shortDate[2]) - 1, shortDate[3], 0, 0, 0, 0); // return specified date at 00:00:00:000
  }
