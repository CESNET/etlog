...
function get_logs($scope, $http, qs, callback)
{
  ...
  .then(function(response) {
    $scope.download_url = '/api/download/search/' + qs + ts + $scope.sort;   // set download url every time - params could have changed
...
function get_mac_count($scope, $http, qs, callback)
{
  ...
  .then(function(response) {
    $scope.download_url = '/api/download/mac_count/' + qs + ts + "&sort=-count";   // set download url every time - params could have changed
...
function get_shared_mac($scope, $http, qs, callback)
{
  ...
  .then(function(response) {
    $scope.download_url = '/api/download/shared_mac/' + qs + ts + "&sort=-count";   // set download url every time - params could have changed
    $scope.table_data = response.data;
    callback($scope);
  });
}
...
{
  $scope.download_data = function() {
    console.log($scope.download_url);
...
function get_concurrent_users($scope, $http, qs, callback)
{
  ...
  .then(function(response) {
    $scope.download_url = '/api/download/concurrent_users/' + qs + ts + $scope.sort;   // set download url every time - params could have changed
